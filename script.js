...
// ==========================================================
// SOCIAL / CHAT PRIVADO ESTILO WHATSAPP
// ==========================================================
...
async function carregarConversasSocial(){let c=document.getElementById('socialConversas');if(!c||!clienteLogado)return;try{let sn=await db.collection('chats').get();let conv=[];sn.forEach(d=>{let data=d.data();if(d.id.includes(clienteLogado.id)&&data.mensagens&&data.mensagens.length>0){conv.push({id:d.id,ultima:data.mensagens[data.mensagens.length-1],total:data.mensagens.length});}});if(conv.length===0){c.innerHTML='<p style="color:#6B7280;">Nenhuma conversa</p>';return;}c.innerHTML=conv.map(x=>'<div style="padding:12px;background:rgba(255,255,255,.03);border-radius:10px;margin-bottom:6px;cursor:pointer;" onclick="abrirChatPorId(\''+x.id+'\')"><div style="display:flex;align-items:center;gap:10px;"><div style="width:45px;height:45px;border-radius:50%;background:linear-gradient(135deg,#D4A84B,#B8923A);display:flex;align-items:center;justify-content:center;font-size:20px;color:#1a1a1a;">👤</div><div style="flex:1;"><strong style="color:#D4A84B;">'+x.ultima.autor+'</strong><p style="color:#aaa;font-size:12px;">'+(x.ultima.texto||'')+(x.ultima.imagem?' 🖼️ Foto':'')+(x.ultima.audio?' 🎤 Áudio':'')+'</p></div><span style="font-size:10px;color:#666;">'+x.total+'</span></div></div>').join('');}catch(e){}}
...
// CHAT PRIVADO - ESTILO WHATSAPP COM CORES DO TEMA
async function carregarMensagensChat(cid){
    ...
    try{
        ...
        msgs.forEach(m=>{
            ...
            if(msgDate!==lastDate){
                html+='<div style="text-align:center;margin:10px 0;"><span style="background:#1a1a1a;color:#aaa;padding:4px 12px;border-radius:10px;font-size:11px;">'+msgDate+'</span></div>';
                lastDate=msgDate;
            }
            let isSent=m.autorId===meuId;
            let time=new Date(m.data).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});
            if(isSent){
                html+='<div style="display:flex;justify-content:flex-end;margin:3px 0;">';
                html+='<div style="max-width:75%;background:linear-gradient(135deg,#D4A84B,#B8923A);color:#1a1a1a;padding:8px 12px;border-radius:12px 12px 0 12px;font-size:13px;line-height:1.4;position:relative;">';
                ...
                html+='<div style="text-align:right;font-size:9px;opacity:0.7;margin-top:2px;">'+time+'</div>';
                html+='</div></div>';
            }else{
                html+='<div style="display:flex;justify-content:flex-start;margin:3px 0;">';
                html+='<div style="max-width:75%;background:#2a2a2a;color:#fff;padding:8px 12px;border-radius:12px 12px 12px 0;font-size:13px;line-height:1.4;position:relative;">';
                html+='<div style="font-size:10px;color:#D4A84B;font-weight:600;margin-bottom:2px;">'+m.autor+'</div>';
                if(m.imagem)html+='<img src="'+m.imagem+'" style="max-width:200px;border-radius:8px;margin-bottom:4px;cursor:pointer;" onclick="window.open(this.src)">';
                if(m.audio)html+='<audio controls src="'+m.audio+'" style="height:30px;width:180px;"></audio>';
                if(m.texto)html+=m.texto;
                html+='<div style="text-align:right;font-size:9px;opacity:0.5;margin-top:2px;">'+time+'</div>';
                html+='</div></div>';
            }
        });
        c.innerHTML=html;
...
async function enviarAudioLive(base64Audio){let nome='Visitante',foto='logobarbearia-rm.png';if(clienteLogado){nome=clienteLogado.nome;foto=clienteLogado.fotoPerfil||'logobarbearia-rm.png';}else if(barbeiroLogado){nome='✂️ '+barbeiroLogado.nome;foto=barbeiroLogado.fotoPerfil||'logobarbearia-rm.png';}try{await db.collection('lives').doc('live_atual').update({chat:firebase.firestore.FieldValue.arrayUnion({autor:nome,foto,audio:base64Audio,data:new Date().toISOString()})});}catch(e){}}
async function iniciarGravacaoAudioPrivado(){if(gravandoAudio)return;try{let stream=await navigator.mediaDevices.getUserMedia({audio:!0});mediaRecorderAudio=new MediaRecorder(stream);audioChunks=[];mediaRecorderAudio.ondataavailable=(e)=>{if(e.data.size>0)audioChunks.push(e.data);};mediaRecorderAudio.onstop=async()=>{let audioBlob=new Blob(audioChunks,{type:'audio/webm'});let reader=new FileReader();reader.onload=async(e)=>{let cid=document.getElementById('chatClienteId').value;let meuId=clienteLogado?clienteLogado.id:(barbeiroLogado?barbeiroLogado.id:'');let meuNome=clienteLogado?clienteLogado.nome:(barbeiroLogado?barbeiroLogado.nome:'Anônimo');let chatId=[meuId,cid].sort().join('_');let doc=await db.collection('chats').doc(chatId).get();let msgs=doc.exists?(doc.data().mensagens||[]):[];msgs.push({autor:meuNome,autorId:meuId,audio:e.target.result,data:new Date().toISOString()});await db.collection('chats').doc(chatId).set({mensagens:msgs});carregarMensagensChat(cid);mostrarToast('🎤 Áudio enviado!','success');};reader.readAsDataURL(audioBlob);stream.getTracks().forEach(t=>t.stop());};mediaRecorderAudio.start();gravandoAudio=!0;let btnMic=document.getElementById('btnMicPrivado');if(btnMic){btnMic.classList.add('recording');btnMic.textContent='⏹️';}mostrarToast('🎤 Gravando...','info');setTimeout(()=>{if(gravandoAudio)pararGravacaoAudioPrivado();},30000);}catch(e){mostrarToast('❌ Permita o microfone!','error');}}
...
function iniciarVisualizacaoCliente(){let oldFrame=document.getElementById('liveFrameImg');if(oldFrame)oldFrame.remove();let frameImg=document.createElement('img');frameImg.id='liveFrameImg';frameImg.style.cssText='position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;z-index:1;';let playerDiv=document.getElementById('livePlayerContainer').querySelector('div');playerDiv.appendChild(frameImg);let frameInterval=setInterval(async()=>{try{let doc=await db.collection('lives').doc('live_atual').get();if(!doc.exists||!doc.data().ativa){clearInterval(frameInterval);if(frameImg)frameImg.remove();document.getElementById('livePlaceholder').style.display='flex';document.getElementById('liveStatus').style.display='none';liveAtivaGeral=!1;return;}let frame=doc.data().ultimoFrame;if(frame&&frameImg)frameImg.src=frame;document.getElementById('liveViewerCount').textContent='👥 '+(doc.data().viewers||0);document.getElementById('liveLikeCount').textContent=doc.data().likes||0;}catch(e){}},1500);db.collection('lives').doc('live_atual').update({viewers:firebase.firestore.FieldValue.increment(1)});iniciarChatListenerLive();}
function iniciarChatListenerLive(){if(liveChatListener)liveChatListener();liveChatListener=db.collection('lives').doc('live_atual').onSnapshot((doc)=>{if(!doc.exists||!doc.data().ativa)return;let data=doc.data();let chat=data.chat||[];let container=document.getElementById('liveChatMessages');if(!container)return;let meuNome=clienteLogado?.nome||barbeiroLogado?.nome||'';let html='';if(data.sorteioAtivo){let participantes=data.participantesSorteio||[];html+='<div style="background:rgba(255,215,0,0.2);padding:14px;border-radius:12px;margin:8px 0;border:2px solid #FFD700;text-align:center;align-self:stretch;"><div style="font-size:24px;">🎉</div><strong style="color:#FFD700;font-size:16px;">SORTEIO ATIVO!</strong><br><span style="color:white;">Prêmio: '+data.premioSorteio+'</span><br><span style="font-size:11px;color:#FFD700;">Digite <b>!PARTICIPAR</b></span><br><span style="font-size:12px;color:#fff;">👥 '+participantes.length+' participantes</span></div>';}if(chat.length===0&&!data.sorteioAtivo){html+='<p style="color:#aaa;text-align:center;font-size:12px;align-self:center;">💬 Seja o primeiro!</p>';}chat.forEach(m=>{let isSent=m.autor===meuNome||m.autor==='✂️ '+meuNome;let bubbleClass=isSent?'sent':'received';let time=m.data?new Date(m.data).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):'';if(m.audio){html+='<div class="chat-bubble '+bubbleClass+' audio-bubble">'+(!isSent?'<div class="msg-sender">'+m.autor+'</div>':'')+'<audio controls src="'+m.audio+'" style="height:30px;width:160px;"></audio><span class="msg-time">🎤 '+time+'</span></div>';}else{html+='<div class="chat-bubble '+bubbleClass+'">'+(!isSent?'<div class="msg-sender">'+m.autor+'</div>':'')+m.texto+'<div class="msg-time">'+time+'</div></div>';}});container.innerHTML=html;container.scrollTop=container.scrollHeight;document.getElementById('liveLikeCount').textContent=data.likes||0;document.getElementById('liveViewerCount').textContent='👥 '+(data.viewers||0);});}

// ENVIAR CHAT COM SUPORTE A !PARTICIPAR
...
// CONTINUAÇÃO DAS FUNÇÕES DA LIVE
...
async function notificarClientesWhatsApp(){if(!barbeiroLogado)return;try{let sn=await db.collection('clientes').get();let urlLive=window.location.href.split('?')[0]+'?live=1';let notificados=0;sn.forEach(async(doc)=>{let cliente=doc.data();if(cliente.celular&&cliente.celular.length>=10){let numero=cliente.celular.replace(/\D/g,'');if(numero.length===11)numero='55'+numero;if(numero.length===10)numero='55'+numero;let link='https://wa.me/'+numero+'?text='+encodeURIComponent('🔴 BARBEARIA RM AO VIVO! Assista: '+urlLive);await db.collection('notificacoes').add({clienteId:cliente.id,tipo:'whatsapp',titulo:'📱 Convite',mensagem:'Link gerado',link:link,lida:!1,data:new Date().toISOString()});notificados++;}});setTimeout(()=>{document.getElementById('whatsappStatus').textContent='✅ '+notificados+' links';document.getElementById('whatsappStatus').style.color='#00cc66';},2000);}catch(e){}}
async function enviarConvitesWhatsApp(){if(!barbeiroLogado)return;if(!confirm('Abrir WhatsApp para todos?'))return;try{let sn=await db.collection('clientes').get();let urlLive=window.location.href.split('?')[0]+'?live=1';let c=0;for(let doc of sn.docs){let cl=doc.data();if(cl.celular&&cl.celular.length>=10){let n=cl.celular.replace(/\D/g,'');if(n.length===11)n='55'+n;if(c<5)window.open('https://wa.me/'+n+'?text='+encodeURIComponent('🔴 LIVE! Assista: '+urlLive),'_blank');c++;await new Promise(r=>setTimeout(r,300));}}mostrarToast('📱 '+c+' convites!','success');}catch(e){}}
