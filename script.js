...
function carregarSessao() {
    ...
    try {
        ...
        if ((new Date().getTime() - sessao.timestamp) / (1000 * 60 * 60 * 24) > 30) { localStorage.removeItem('barbeariaRM_sessao'); return null; }
        return sessao;
...
async function cadastrarCliente() {
    ...
    try {
        const snap = await db.collection('clientes').where('email', '==', e).get();
        if (!snap.empty) { mostrarToast('❌ E-mail já cadastrado!', 'error'); return; }
        var id = Date.now().toString();
        var cliente = { id, nome: n, email: e, celular: c, senha: s, fotoPerfil: '', dataCriacao: new Date().toISOString() };
...
async function cadastrarBarbeiro() {
    ...
    if (s.length < 6) { mostrarToast('❌ Senha mínima 6 caracteres!', 'error'); return; }
    try {
        const snap = await db.collection('barbeiros').where('email', '==', e).get();
        if (!snap.empty) { mostrarToast('❌ E-mail já cadastrado!', 'error'); return; }
        var id = Date.now().toString();
        var barbeiro = { id, nome: n, email: e, celular: c, senha: s, fotoPerfil: '', dataCriacao: new Date().toISOString() };
...
async function loginCliente() {
    ...
    try {
        const snap = await db.collection('clientes').where('email', '==', e).where('senha', '==', s).get();
...
async function loginBarbeiro() {
    ...
    if (!e || !s) { mostrarToast('❌ Preencha todos os campos!', 'error'); return; }
    try {
        const snap = await db.collection('barbeiros').where('email', '==', e).where('senha', '==', s).get();
        if (snap.empty) { mostrarToast('❌ E-mail ou senha inválidos!', 'error'); return; }
...
async function criarPost() {
    ...
    try {
        ...
        await db.collection('posts').doc(id).set({
            id, barbeiroId: barbeiroLogado.id, barbeiroNome: barbeiroLogado.nome,
...
async function agendarCorte() {
    ...
    try {
        ...
        await db.collection('agendamentos').doc(id).set({
            id, clienteId: clienteLogado.id, clienteNome: clienteLogado.nome, clienteEmail: clienteLogado.email,
            data, horario, tipo, status: 'pendente', dataCriacao: new Date().toISOString()
        });
