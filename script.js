...
async function iniciarLive(){
    ...
    try{
        ...
        liveFrameCanvas=document.createElement('canvas');liveFrameCanvas.width=480;liveFrameCanvas.height=270;
...
function alternarParaTela1(){if(!barbeiroLogado){mostrarToast('❌ Apenas barbeiro!','error');return;}document.getElementById('liveTela1').style.display='block';document.getElementById('liveTela2').style.display='none';liveTelaAtiva=1;document.getElementById('liveTelaIndicador').textContent='🎥 CÂMERA';mostrarToast('🎥 Câmera ativada','info');db.collection('lives').doc('live_atual').update({telaAtiva:1}).catch(function(){});}
function alternarParaTela2(){if(!barbeiroLogado){mostrarToast('❌ Apenas barbeiro!','error');return;}document.getElementById('liveTela1').style.display='none';document.getElementById('liveTela2').style.display='block';liveTelaAtiva=2;document.getElementById('liveTelaIndicador').textContent='📺 COMERCIAL';mostrarToast('📺 Comercial ativado','info');db.collection('lives').doc('live_atual').update({telaAtiva:2}).catch(function(){});}

function carregarVideoComercial(event){
    var file=event.target.files[0];if(!file)return;
    if(file.size>500000){mostrarToast('❌ Vídeo grande! Máx 500KB','error');return;}
    ...
    document.getElementById('comercialStatus').textContent='✅ '+file.name;mostrarToast('🎬 Vídeo carregado!','success');
}
function carregarImagemComercial(event){
    var file=event.target.files[0];if(!file)return;
    var reader=new FileReader();
    reader.onload=function(e){
        if(e.target.result.length>500000){mostrarToast('❌ Imagem grande!','error');return;}
        ...
        document.getElementById('comercialStatus').textContent='✅ '+file.name;mostrarToast('🖼️ Imagem carregada!','success');
    };reader.readAsDataURL(file);
}
function adicionarLinkComercial(){
    var link=prompt('Cole o link do vídeo (YouTube, URL direta, etc.):');
    if(!link)return;
    document.getElementById('liveImagemComercial').style.display='none';document.getElementById('liveTela2Placeholder').style.display='none';
    if(link.includes('youtube.com')||link.includes('youtu.be')){
        ...
        var iframe=document.createElement('iframe');iframe.src=link;iframe.style.cssText='position:absolute;top:0;left:0;width:100%;height:100%;border:none;';iframe.allow='autoplay;encrypted-media';iframe.allowFullscreen=true;
        var container=document.getElementById('liveTela2');var oldIframe=container.querySelector('iframe');if(oldIframe)oldIframe.remove();
        container.appendChild(iframe);
        document.getElementById('comercialStatus').textContent='✅ YouTube';
    }else{
        ...
        var container=document.getElementById('liveTela2');var oldIframe=container.querySelector('iframe');if(oldIframe)oldIframe.remove();
        document.getElementById('comercialStatus').textContent='✅ Link';
    }
...
function removerVideoComercial(){
    ...
    document.getElementById('comercialStatus').textContent='Nenhum';
    var container=document.getElementById('liveTela2');var oldIframe=container.querySelector('iframe');if(oldIframe)oldIframe.remove();
...
function removerAnuncioLive(){document.getElementById('liveAnuncioOverlay').style.display='none';liveAnuncioAtivo=false;}
...
async function carregarLive(){
    ...
    try{
        ...
        if(doc.exists&&doc.data().ativa){
            ...
            if(!barbeiroLogado||barbeiroLogado.id!==live.barbeiroId){document.getElementById('liveTelaIndicador').style.display='none';}else{document.getElementById('liveTelaIndicador').style.display='block';}
            ...
            if(barbeiroLogado&&barbeiroLogado.id===live.barbeiroId){
                ...
                if(liveLocalStream){document.getElementById('liveVideo').srcObject=liveLocalStream;document.getElementById('liveVideo').style.display='block';document.getElementById('liveVideo').muted=true;document.getElementById('liveFrameImg').style.display='none';document.getElementById('liveVideoLocal').style.display='block';}
