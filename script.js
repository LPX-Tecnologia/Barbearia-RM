...
function carregarDadosTela(nomeTela) {
    switch(nomeTela) {
        ...
        case 'live': break; // Live tem seus próprios listeners
...
function carregarSessao() {
    ...
    if (!d) return null;
    try { var s = JSON.parse(d); if ((Date.now()-s.timestamp)/86400000 > 30) { limparSessao(); return null; } return s; }
...
async function restaurarSessao() {
    var s = carregarSessao(); if (!s) return false;
    try {
        if (s.tipo === 'cliente') {
            var sn = await db.collection('clientes').where('email','==',s.email).where('senha','==',s.senha).get();
            if (!sn.empty) { var d = sn.docs[0]; clienteLogado = { id: d.id, ...d.data() }; document.getElementById('welcomeClienteNome').textContent = clienteLogado.nome; mostrarTela('homeCliente'); return true; }
        } else {
            var sn = await db.collection('barbeiros').where('email','==',s.email).where('senha','==',s.senha).get();
            if (!sn.empty) { var d = sn.docs[0]; barbeiroLogado = { id: d.id, ...d.data() }; document.getElementById('welcomeBarbeiroNome').textContent = barbeiroLogado.nome; mostrarTela('homeBarbeiro'); return true; }
        }
    } catch(e) {}
...
async function cadastrarCliente() {
    ...
    try {
        var sn = await db.collection('clientes').where('email','==',e).get(); if (!sn.empty) { mostrarToast('❌ Email existe!', 'error'); return; }
        var id = Date.now().toString(), cl = { id, nome:n, email:e, celular:c, senha:s, fotoPerfil:'', dataCriacao: new Date().toISOString() };
        ...
        mostrarToast('✅ Cadastro OK!', 'success'); mostrarTela('homeCliente');
    } catch(er) { mostrarToast('❌ Erro!', 'error'); }
}
...
async function cadastrarBarbeiro() {
    ...
    if (s.length < 6) { mostrarToast('❌ Senha 6+', 'error'); return; }
    try {
        var sn = await db.collection('barbeiros').where('email','==',e).get(); if (!sn.empty) { mostrarToast('❌ Email existe!', 'error'); return; }
        var id = Date.now().toString(), bb = { id, nome:n, email:e, celular:c, senha:s, fotoPerfil:'', dataCriacao: new Date().toISOString() };
        ...
        mostrarToast('✅ Cadastro OK!', 'success'); mostrarTela('homeBarbeiro');
    } catch(er) { mostrarToast('❌ Erro!', 'error'); }
}

function sairCliente() { clienteLogado = null; limparSessao(); fecharFormulariosLogin(); mostrarTela('login'); mostrarToast('👋 Até logo!', 'info'); }
function sairBarbeiro() { barbeiroLogado = null; limparSessao(); fecharFormulariosLogin(); mostrarTela('login'); mostrarToast('👋 Até logo!', 'info'); }
...
async function carregarMeusPosts() {
    ...
    try {
        var sn = await db.collection('posts').where('barbeiroId','==',barbeiroLogado.id).get(); var posts = [];
...
async function carregarComentarios(id) {
    ...
    c.innerHTML = com.length===0 ? '<p style="color:#6B7280;">Nenhum</p>' : com.map(function(x){ return '<div style="padding:8px;margin:4px 0;background:rgba(255,255,255,.03);border-radius:8px;"><strong style="color:#D4A84B;">'+x.autor+'</strong><p style="color:#B0B0B0;">'+x.texto+'</p></div>'; }).join('');
}
...
async function adicionarComentario() {
    ...
    var doc = await db.collection('posts').doc(postSelecionadoId).get(); var com = doc.data().comentarios||[];
...
function carregarOpcoesAgendamento() {
    var sh=document.getElementById('agendamentoHorario'), st=document.getElementById('agendamentoTipo');
    if (sh) { var hor=[]; for(var h=9;h<=18;h++){ for(var m=0;m<60;m+=30){ if(h===18&&m>0)break; hor.push(String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')); } } sh.innerHTML = hor.map(function(x){ return '<option value="'+x+'">'+x+'</option>'; }).join(''); }
    if (st) { var tipos=['Corte Social','Corte Degradê','Corte Navalhado','Corte Máquina','Barba','Barba + Corte']; st.innerHTML = tipos.map(function(x){ return '<option value="'+x+'">'+x+'</option>'; }).join(''); }
    var id=document.getElementById('agendamentoData'); if(id) id.min = new Date().toISOString().split('T')[0];
}

async function agendarCorte() {
    if (!clienteLogado) { mostrarToast('❌ Faça login!', 'error'); return; }
    var d=document.getElementById('agendamentoData').value, h=document.getElementById('agendamentoHorario').value, t=document.getElementById('agendamentoTipo').value;
...
async function carregarAgendaCliente() {
    if (!clienteLogado) return; var c=document.getElementById('agendaClienteContainer'); if (!c) return;
    try {
        var sn=await db.collection('agendamentos').where('clienteId','==',clienteLogado.id).get();
        var ag=sn.docs.map(function(d){ return { id:d.id, ...d.data() }; }); ag.sort(function(a,b){ return new Date(b.data)-new Date(a.data); });
        if (ag.length===0) { c.innerHTML='<p style="color:#6B7280;text-align:center;">Nenhum</p>'; return; }
        c.innerHTML = ag.map(function(a){ return '<div class="agenda-item"><div class="agenda-info"><div class="agenda-cliente">'+a.tipo+'</div><div class="agenda-data">📅 '+a.data+' • ⏰ '+a.horario+'</div></div><span class="agenda-status '+a.status+'">'+(a.status==='confirmado'?'✅':'⏳')+' '+a.status+'</span></div>'; }).join('');
    } catch(e) { c.innerHTML='<p style="color:#6B7280;">Erro</p>'; }
}
...
async function carregarAgendamentosBarbeiro() {
    ...
    try {
        ...
        if (ag.length===0) { c.innerHTML='<p style="color:#6B7280;text-align:center;">Nenhum</p>'; return; }
        c.innerHTML = ag.map(function(a){ return '<div class="agenda-item"><div class="agenda-info"><div class="agenda-cliente">👤 '+(a.clienteNome||'Cliente')+'</div><div class="agenda-data">📅 '+a.data+' • ⏰ '+a.horario+' • ✂️ '+a.tipo+'</div></div><span class="agenda-status '+a.status+'">'+a.status+'</span></div>'; }).join('');
    } catch(e) { c.innerHTML='<p style="color:#6B7280;">Erro</p>'; }
}
...
async function carregarPlanos() {
    ...
    try {
        ...
        c.innerHTML = p.map(function(x){ return '<div class="plano-card"><div class="plano-info"><div class="plano-nome">'+x.nome+'</div><div class="plano-periodo">📅 '+x.periodo+'</div></div><div class="plano-preco">R$ '+(x.preco||0).toFixed(2)+'</div></div>'; }).join('');
    } catch(e) { c.innerHTML='<p style="color:#6B7280;">Erro</p>'; }
}
...
async function salvarPerfilCliente() {
    if (!clienteLogado) return;
    var n=document.getElementById('editClienteNome').value.trim(), c=document.getElementById('editClienteCelular').value.trim();
...
async function salvarPerfilBarbeiro() {
    if (!barbeiroLogado) return;
    var n=document.getElementById('editBarbeiroNome').value.trim(), c=document.getElementById('editBarbeiroCelular').value.trim(), e=document.getElementById('editBarbeiroEmail').value.trim();
...
async function calcularFaturamento() {
    try {
        ...
        var hoje=new Date().toISOString().split('T')[0]; var th=0, tg=0;
...
async function carregarLive() {
    // A live é gerenciada pelos próprios event listeners
    console.log('📺 Tela de live carregada');
}
...
async function iniciarLive() {
    try {
        ...
        await db.collection('lives').doc('live_atual').set({
            id: 'live_atual', titulo: 'Live Barbearia RM', barbeiroNome: 'Barbearia RM',
            ativa: true, chat: [], viewers: 0, likes: 0, dataInicio: new Date().toISOString()
        });
...
function atualizarTimelineLive() {
    ...
    tl.innerHTML = trechosGravados.map((c, i) => 
        `<span style="background:rgba(255,255,255,.05);padding:4px 8px;border-radius:6px;font-size:10px;cursor:pointer;margin-right:4px;" onclick="window.open('${c.url}','_blank')">🎬 ${i+1}</span>`
    ).join('');
}
...
async function trocarCameraLive() {
    ...
    } catch(e) { mostrarToast('❌ Erro ao trocar', 'error'); }
}
...
function setupEventListeners() {
    ...
    // Login
    document.getElementById('btnMostrarLoginCliente')?.addEventListener('click', function(){ mostrarFormularioLogin('cliente'); });
    document.getElementById('btnMostrarLoginBarbeiro')?.addEventListener('click', function(){ mostrarFormularioLogin('barbeiro'); });
    ...
    document.getElementById('fotoClienteInput')?.addEventListener('change', function(e){
        ...
        r.onload=async function(ev){ var foto=ev.target.result; document.querySelector('#perfilClienteAvatar img').src=foto; if(clienteLogado){clienteLogado.fotoPerfil=foto; await db.collection('clientes').doc(clienteLogado.id).update({fotoPerfil:foto}); salvarSessao('cliente',clienteLogado);} };
        r.readAsDataURL(f);
    });
    document.getElementById('fotoBarbeiroInput')?.addEventListener('change', function(e){
        var f=e.target.files[0]; if(!f)return; var r=new FileReader();
        r.onload=async function(ev){ var foto=ev.target.result; document.querySelector('#perfilBarbeiroAvatar img').src=foto; if(barbeiroLogado){barbeiroLogado.fotoPerfil=foto; await db.collection('barbeiros').doc(barbeiroLogado.id).update({fotoPerfil:foto}); salvarSessao('barbeiro',barbeiroLogado);} };
        r.readAsDataURL(f);
    });
    ...
    document.getElementById('btnTelaCamera')?.addEventListener('click', function(){ mostrarTelaLive('camera'); });
    document.getElementById('btnTelaComercial')?.addEventListener('click', function(){ mostrarTelaLive('comercial'); });
    document.getElementById('btnTelaAnuncio')?.addEventListener('click', function(){ mostrarTelaLive('anuncio'); });
...
document.addEventListener('DOMContentLoaded', async function(){
    ...
    if (!restaurado) { document.getElementById('loginScreen').classList.add('active'); console.log('👋 Nenhuma sessão'); }
