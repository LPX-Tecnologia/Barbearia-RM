...
// ==========================================================
// ===== LIVE (CORRIGIDA - CLIENTE VÊ FRAME) =====
// ==========================================================
function gerarViewerId(){return'viewer_'+Date.now()+'_'+Math.random().toString(36).substr(2,9);}
async function adicionarViewerLive(){if(!liveAtiva||liveViewerId)return;liveViewerId=gerarViewerId();var nome=clienteLogado?'💇 '+clienteLogado.nome:(barbeiroLogado?'✂️ '+barbeiroLogado.nome:'👤 Visitante');var doc=await db.collection('lives').doc('live_atual').get();if(!doc.exists||!doc.data().ativa)return;var va=doc.data().viewersAtivos||{};va[liveViewerId]={nome:nome,entrou:new Date().toISOString(),ultimoPing:new Date().toISOString()};var agora=Date.now(),vl={},count=0;Object.keys(va).forEach(function(k){if(agora-new Date(va[k].ultimoPing).getTime()<45000){vl[k]=va[k];count++;}});await db.collection('lives').doc('live_atual').update({viewersAtivos:vl,viewers:count,totalViews:firebase.firestore.FieldValue.increment(1)});iniciarViewerPing();}
function iniciarViewerPing(){if(liveViewerInterval)clearInterval(liveViewerInterval);liveViewerInterval=setInterval(async function(){if(!liveAtiva||!liveViewerId){clearInterval(liveViewerInterval);return;}var doc=await db.collection('lives').doc('live_atual').get();if(!doc.exists||!doc.data().ativa){removerViewerLive();return;}var va=doc.data().viewersAtivos||{};if(va[liveViewerId]){va[liveViewerId].ultimoPing=new Date().toISOString();await db.collection('lives').doc('live_atual').update({viewersAtivos:va});}},15000);}
async function removerViewerLive(){if(liveViewerInterval){clearInterval(liveViewerInterval);liveViewerInterval=null;}if(!liveViewerId)return;var doc=await db.collection('lives').doc('live_atual').get();if(!doc.exists)return;var va=doc.data().viewersAtivos||{};delete va[liveViewerId];var count=Object.keys(va).length;await db.collection('lives').doc('live_atual').update({viewersAtivos:va,viewers:count});liveViewerId=null;}
...
async function iniciarLive(){
    ...
    try{
        var stream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:640},height:{ideal:360},facingMode:'user'},audio:true});
        ...
        await db.collection('lives').doc('live_atual').set({id:'live_atual',barbeiroId:barbeiroLogado.id,barbeiroNome:barbeiroLogado.nome,titulo,ativa:true,chat:[],viewers:0,viewersAtivos:{},totalViews:0,likes:0,dataInicio:new Date().toISOString()});
        ...
        atualizarChat();iniciarChatListener();verificarLiveAtiva();adicionarViewerLive();
        // Capturar frames para clientes
        setInterval(async function(){if(!liveAtiva||!liveLocalStream)return;var v=document.getElementById('liveVideo');if(!v||!v.videoWidth)return;var c=document.createElement('canvas');c.width=320;c.height=180;var ctx=c.getContext('2d');ctx.drawImage(v,0,0,320,180);var frame=c.toDataURL('image/jpeg',0.6);await db.collection('lives').doc('live_atual').update({ultimoFrame:frame,frameTimestamp:new Date().toISOString()}).catch(function(){});},2000);
...
async function carregarLive(){
    ...
    try{
        ...
        if(doc.exists&&doc.data().ativa){
            ...
            if(status){status.style.display='block';document.getElementById('liveStatusTitulo').textContent=live.titulo;var va=live.viewersAtivos||{};var count=Object.keys(va).length;document.getElementById('liveStatusViewers').textContent='👥 '+count+' • 👁 '+(live.totalViews||0)+' • ❤️ '+(live.likes||0);}
            ...
            if(barbeiroLogado&&barbeiroLogado.id===live.barbeiroId){
                ...
                if(player){player.style.display='block';document.getElementById('liveVideo').srcObject=liveLocalStream;document.getElementById('liveVideo').style.display='block';document.getElementById('liveFrameImg').style.display='none';document.getElementById('liveVideoLocal').style.display='block';}
                if(warning)warning.style.display='none';
                document.getElementById('liveViewerCount').textContent='👥 '+count+' • 👁 '+(live.totalViews||0);
            }else if(clienteLogado){
                if(controls)controls.style.display='none';
                if(player){player.style.display='block';document.getElementById('liveVideo').style.display='none';document.getElementById('liveVideoLocal').style.display='none';
                    if(live.ultimoFrame){document.getElementById('liveFrameImg').src=live.ultimoFrame;document.getElementById('liveFrameImg').style.display='block';}
                }
                if(warning)warning.style.display='none';
...
// --- CHAT ---
function iniciarChatListener(){pararChatListener();liveChatInterval=setInterval(async function(){var doc=await db.collection('lives').doc('live_atual').get();if(doc.exists&&doc.data().ativa){var novas=doc.data().chat||[];if(novas.length!==liveChatMessages.length){liveChatMessages=novas;atualizarChat();}var va=doc.data().viewersAtivos||{};var count=Object.keys(va).length;var el=document.getElementById('liveStatusViewers');var el2=document.getElementById('liveViewerCount');if(el)el.textContent='👥 '+count+' • 👁 '+(doc.data().totalViews||0)+' • ❤️ '+(doc.data().likes||0);if(el2)el2.textContent='👥 '+count+' • 👁 '+(doc.data().totalViews||0);}},3000);}
...
async function enviarMensagemLive(){
    ...
    var autor=clienteLogado?'💇 '+clienteLogado.nome:(barbeiroLogado?'✂️ '+barbeiroLogado.nome:'👤 Visitante');
    var doc=await db.collection('lives').doc('live_atual').get();
...
function atualizarChat(){
    ...
    c.innerHTML=liveChatMessages.map(function(msg){var h=new Date(msg.data).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});return'<div class="live-chat-message"><span class="autor">'+(msg.autor||'Anônimo')+'</span> <span style="font-size:10px;color:#6B7280;">'+h+'</span><div class="texto">'+msg.texto+'</div></div>';}).join('');c.scrollTop=c.scrollHeight;
}
...
// ==========================================================
// ===== FEED / POSTS / PERFIL / HORÁRIOS (COMPLETO) =====
// ==========================================================
