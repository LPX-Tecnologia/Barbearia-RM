...
// ==========================================================
// ===== AGENDAMENTOS / PLANOS / ANÚNCIOS / FEED / POSTS =====
// ===== (MANTIDOS IGUAIS - CÓDIGO COMPLETO) =====
// ==========================================================
...
async function iniciarLive(){
    ...
    try{
        var stream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:640},height:{ideal:480},facingMode:'user',frameRate:{ideal:30}},audio:{echoCancellation:false,noiseSuppression:false,autoGainControl:false,sampleRate:48000}});
        ...
        document.getElementById('liveVideo').style.width='100%';document.getElementById('liveVideo').style.height='100%';document.getElementById('liveVideo').style.objectFit='cover';
        ...
        await db.collection('lives').doc('live_atual').set({id:'live_atual',barbeiroId:barbeiroLogado.id,barbeiroNome:barbeiroLogado.nome,titulo,ativa:true,chat:[],viewers:0,totalViews:0,likes:0,telaAtiva:1,comercialVideo:'',comercialTipo:'nenhum',comercialLink:'',anuncioAtivo:false,anuncioImagem:'',dataInicio:new Date().toISOString()});
        ...
        document.getElementById('liveTela1').style.display='block';document.getElementById('liveTela2').style.display='none';
        document.getElementById('liveTelaIndicador').textContent='🎥 TELA 1 ATIVA';
        // Esconder indicadores para cliente
        document.getElementById('liveTelaIndicador').style.display=barbeiroLogado?'block':'none';
        document.querySelectorAll('.live-tela-label').forEach(function(el){el.style.display=barbeiroLogado?'block':'none';});
        liveFrameCanvas=document.createElement('canvas');liveFrameCanvas.width=480;liveFrameCanvas.height=270;liveFrameCtx=liveFrameCanvas.getContext('2d');
...
function iniciarCapturaFrames(){if(liveFrameInterval)clearInterval(liveFrameInterval);liveFrameInterval=setInterval(async function(){if(!liveAtiva||!liveLocalStream||!liveFrameCtx)return;try{var video=document.getElementById('liveVideo');if(!video||!video.videoWidth||video.videoWidth===0)return;var agora=Date.now();if(agora-liveUltimoFrameEnviado<250)return;liveUltimoFrameEnviado=agora;liveFrameCtx.drawImage(video,0,0,480,270);var frameData=liveFrameCanvas.toDataURL('image/jpeg',0.6);db.collection('lives').doc('live_atual').update({ultimoFrame:frameData,frameTimestamp:agora}).catch(function(){});}catch(e){}},250);}

function alternarParaTela1(){if(!barbeiroLogado){mostrarToast('❌ Apenas barbeiro!','error');return;}document.getElementById('liveTela1').style.display='block';document.getElementById('liveTela2').style.display='none';liveTelaAtiva=1;document.getElementById('liveTelaIndicador').textContent='🎥 TELA 1 ATIVA (Câmera)';mostrarToast('🎥 Câmera ativada','info');db.collection('lives').doc('live_atual').update({telaAtiva:1}).catch(function(){});}
function alternarParaTela2(){if(!barbeiroLogado){mostrarToast('❌ Apenas barbeiro!','error');return;}document.getElementById('liveTela1').style.display='none';document.getElementById('liveTela2').style.display='block';liveTelaAtiva=2;document.getElementById('liveTelaIndicador').textContent='📺 TELA 2 ATIVA (Comercial)';mostrarToast('📺 Comercial ativado','info');db.collection('lives').doc('live_atual').update({telaAtiva:2}).catch(function(){});}

// --- VÍDEO/LINK COMERCIAL ---
function carregarVideoComercial(event){
    var file=event.target.files[0];if(!file)return;
    if(file.size>500000){mostrarToast('❌ Vídeo muito grande! Máx 500KB. Use um link.', 'error');return;}
    var url=URL.createObjectURL(file);
    document.getElementById('liveVideoComercial').src=url;document.getElementById('liveVideoComercial').loop=true;document.getElementById('liveVideoComercial').muted=true;
    document.getElementById('liveVideoComercial').style.display='block';document.getElementById('liveImagemComercial').style.display='none';document.getElementById('liveTela2Placeholder').style.display='none';
    document.getElementById('comercialStatus').textContent='✅ Arquivo: '+file.name;
    mostrarToast('🎬 Vídeo carregado!','success');
    // NÃO salvar no Firestore (evita erro de tamanho)
    db.collection('lives').doc('live_atual').update({comercialTipo:'video_arquivo',comercialLink:''}).catch(function(){});
}
function carregarImagemComercial(event){
    var file=event.target.files[0];if(!file)return;
    var reader=new FileReader();
    reader.onload=function(e){
        if(e.target.result.length>500000){mostrarToast('❌ Imagem muito grande! Use uma menor.', 'error');return;}
        document.getElementById('liveImagemComercial').src=e.target.result;
        document.getElementById('liveImagemComercial').style.display='block';document.getElementById('liveVideoComercial').style.display='none';document.getElementById('liveTela2Placeholder').style.display='none';
        document.getElementById('comercialStatus').textContent='✅ Imagem: '+file.name;
...
function adicionarLinkComercial(){
    ...
    if(link.includes('youtube.com')||link.includes('youtu.be')){
        ...
        if(videoId){videoId=videoId.split('&')[0];link='https://www.youtube.com/embed/'+videoId+'?autoplay=1&mute=1&loop=1&playlist='+videoId;}
        document.getElementById('liveVideoComercial').style.display='none';
        var iframe=document.createElement('iframe');
        iframe.src=link;iframe.style.cssText='position:absolute;top:0;left:0;width:100%;height:100%;border:none;';iframe.allow='autoplay;encrypted-media';iframe.allowFullscreen=true;
        var container=document.getElementById('liveTela2').querySelector('div');
        var oldIframe=container.querySelector('iframe');if(oldIframe)oldIframe.remove();
        container.appendChild(iframe);
        document.getElementById('comercialStatus').textContent='✅ YouTube: '+link.substring(0,40)+'...';
    }else{
        document.getElementById('liveVideoComercial').src=link;document.getElementById('liveVideoComercial').loop=true;document.getElementById('liveVideoComercial').muted=true;
        document.getElementById('liveVideoComercial').style.display='block';
        var container=document.getElementById('liveTela2').querySelector('div');var oldIframe=container.querySelector('iframe');if(oldIframe)oldIframe.remove();
        document.getElementById('comercialStatus').textContent='✅ Link: '+link.substring(0,40)+'...';
    }
    mostrarToast('🔗 Link comercial adicionado!','success');
    db.collection('lives').doc('live_atual').update({comercialLink:link,comercialTipo:'link'}).catch(function(){});
}
function removerVideoComercial(){
    document.getElementById('liveVideoComercial').src='';document.getElementById('liveVideoComercial').style.display='none';
    document.getElementById('liveImagemComercial').style.display='none';document.getElementById('liveTela2Placeholder').style.display='flex';
    document.getElementById('comercialStatus').textContent='Nenhum comercial';
    var container=document.getElementById('liveTela2').querySelector('div');var oldIframe=container.querySelector('iframe');if(oldIframe)oldIframe.remove();
    document.getElementById('comercialVideoInput').value='';document.getElementById('comercialImagemInput').value='';
    db.collection('lives').doc('live_atual').update({comercialVideo:'',comercialTipo:'nenhum',comercialLink:''}).catch(function(){});
...
function carregarAnuncioLive(event){
    ...
    reader.onload=function(e){
        ...
        document.getElementById('liveAnuncioOverlay').style.display='flex';liveAnuncioAtivo=true;
        db.collection('lives').doc('live_atual').update({anuncioAtivo:true,anuncioImagem:e.target.result}).catch(function(){});
        mostrarToast('📢 Anúncio exibido!','success');setTimeout(function(){removerAnuncioLive();},10000);
    };reader.readAsDataURL(file);
}
function removerAnuncioLive(){document.getElementById('liveAnuncioOverlay').style.display='none';liveAnuncioAtivo=false;db.collection('lives').doc('live_atual').update({anuncioAtivo:false,anuncioImagem:''}).catch(function(){});}
function fecharAnuncioLive(){removerAnuncioLive();mostrarToast('📢 Removido','info');}
...
async function carregarLive(){
    ...
    try{
        ...
        if(doc.exists&&doc.data().ativa){
            ...
            // Esconder labels de tela para cliente
            if(!barbeiroLogado||barbeiroLogado.id!==live.barbeiroId){document.getElementById('liveTelaIndicador').style.display='none';document.querySelectorAll('.live-tela-label').forEach(function(el){el.style.display='none';});}else{document.getElementById('liveTelaIndicador').style.display='block';}
            if(live.telaAtiva===2){document.getElementById('liveTela1').style.display='none';document.getElementById('liveTela2').style.display='block';}else{document.getElementById('liveTela1').style.display='block';document.getElementById('liveTela2').style.display='none';}
            ...
            if(barbeiroLogado&&barbeiroLogado.id===live.barbeiroId){
                ...
                if(liveLocalStream){document.getElementById('liveVideo').srcObject=liveLocalStream;document.getElementById('liveVideo').style.display='block';document.getElementById('liveVideo').style.width='100%';document.getElementById('liveVideo').style.height='100%';document.getElementById('liveVideo').style.objectFit='cover';document.getElementById('liveVideo').muted=false;document.getElementById('liveFrameImg').style.display='none';document.getElementById('liveVideoLocal').style.display='block';}
