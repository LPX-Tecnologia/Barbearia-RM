...
async function iniciarGravacaoAudioPrivado(){if(gravandoAudio)return;try{let s=await navigator.mediaDevices.getUserMedia({audio:!0});mediaRecorderAudio=new MediaRecorder(s);audioChunks=[];mediaRecorderAudio.ondataavailable=(e)=>{if(e.data.size>0)audioChunks.push(e.data);};mediaRecorderAudio.onstop=async()=>{let blob=new Blob(audioChunks,{type:'audio/webm'});let reader=new FileReader();reader.onload=async(e)=>{let cid=document.getElementById('chatClienteId').value;let meuId=clienteLogado?clienteLogado.id:(barbeiroLogado?barbeiroLogado.id:'');let meuNome=clienteLogado?clienteLogado.nome:(barbeiroLogado?barbeiroLogado.nome:'Anônimo');let chatId=[meuId,cid].sort().join('_');let doc=await db.collection('chats').doc(chatId).get();let msgs=doc.exists?(doc.data().mensagens||[]):[];msgs.push({autor:meuNome,autorId:meuId,audio:e.target.result,data:new Date().toISOString()});await db.collection('chats').doc(chatId).set({mensagens:msgs});carregarMensagensChat(cid);mostrarToast('🎤 Áudio enviado!','success');};reader.readAsDataURL(blob);s.getTracks().forEach(t=>t.stop());};mediaRecorderAudio.start();gravandoAudio=!0;let btn=document.getElementById('btnMicPrivado');if(btn){btn.classList.add('recording');btn.textContent='⏹️';}mostrarToast('🎤 Gravando...','info');setTimeout(()=>{if(gravandoAudio)pararGravacaoAudioPrivado();},30000);}catch(e){mostrarToast('❌ Permita o microfone!','error');}}
...
async function enviarChatLive(){let i=document.getElementById('liveChatInput');if(!i)return;let msg=i.value.trim();if(!msg)return;let nome='Visitante',foto='logobarbearia-rm.png';if(clienteLogado){nome=clienteLogado.nome;foto=clienteLogado.fotoPerfil||'logobarbearia-rm.png';}else if(barbeiroLogado){nome='✂️ '+barbeiroLogado.nome;foto=barbeiroLogado.fotoPerfil||'logobarbearia-rm.png';}try{await db.collection('lives').doc('live_atual').update({chat:firebase.firestore.FieldValue.arrayUnion({autor:nome,foto,texto:msg,data:new Date().toISOString()})});i.value='';if(msg.toUpperCase().trim()==='!PARTICIPAR'){await participarSorteioFirebase(nome);}}catch(e){}}
