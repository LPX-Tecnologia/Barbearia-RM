...
// ===== LOGIN CLIENTE (LOCAL STORAGE - FALLBACK) =====
...
// ===== LOGIN BARBEIRO (LOCAL STORAGE - FALLBACK) =====
...
// ===== CADASTRO CLIENTE (LOCAL STORAGE - FALLBACK) =====
...
async function loginClienteFirebase() {
    ...
    try {
        ...
        if (cliente) {
            await db.collection('clientes').doc(user.uid).update({ ultimoAcesso: Date.now() });
            cliente.ultimoAcesso = Date.now();
            usuarioLogado = cliente;
            tipoUsuario = 'cliente';
            salvarDados('usuarioLogado', cliente);
...
async function loginBarbeiroFirebase() {
    ...
    try {
        ...
        if (barbeiro) {
            await db.collection('barbeiros').doc(user.uid).update({ ultimoAcesso: Date.now() });
...
// ===== CADASTRO CLIENTE (FIREBASE AUTH) =====
...
function logoutFirebase() {
    if (auth) {
        auth.signOut().catch(error => console.error('Erro ao deslogar:', error));
    }
}
...
function carregarAgendamentosBarbeiro() {
    ...
    if (db) {
        ouvirAgendamentosFirebase(renderizarAgendamentosBarbeiro);
    } else {
        const agendamentos = carregarDados('agendamentos') || [];
...
async function atualizarAgendamento() {
    ...
    if (db) {
        try {
            await db.collection('agendamentos').doc(id).update({ data, horario, tipo });
...
function excluirAgendamento(agendamentoId) {
    ...
    if (db) {
        db.collection('agendamentos').doc(agendamentoId).delete()
            .then(() => mostrarToast('🗑️ Agendamento excluído!', 'sucesso'))
            .catch((error) => mostrarToast('❌ Erro ao excluir: ' + error.message, 'erro'));
    } else {
...
function abrirPagamento(postId) {
    ...
    try {
        new QRCode(container, { text: pixString, width: 200, height: 200, colorDark: '#C9A84C', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H });
    } catch (e) {
...
function copiarPix() {
    ...
    navigator.clipboard.writeText(chave).then(() => mostrarToast('Chave PIX copiada!', 'sucesso')).catch(() => mostrarToast('Chave: ' + chave, ''));
}
...
function atualizarLocalizacaoCliente() {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        (error) => console.warn('Erro ao obter localização:', error.message),
