...
async function calcularExtrato(periodo){
    ...
    try{
        ...
        let ag=sn.docs.map(d=>d.data());let hoje=new Date();let total=0;let itens=[];
        ag.forEach(a=>{let dataAg=new Date(a.data);let valor=35;let incluir=!1;
        ...
        document.getElementById('extratoHoje').textContent='R$ '+total.toFixed(2);
        document.getElementById('extratoSemana').textContent='R$ '+(total*0.3).toFixed(2);
        document.getElementById('extratoMes').textContent='R$ '+(total*0.7).toFixed(2);
        document.getElementById('extratoAno').textContent='R$ '+total.toFixed(2);
        if(itens.length===0){container.innerHTML='<p style="color:#6B7280;text-align:center;">Nenhum agendamento</p>';return;}
...
async function carregarMensagensChat(cid){let c=document.getElementById('chatMensagens');if(!c)return;let meuId=clienteLogado?clienteLogado.id:(barbeiroLogado?barbeiroLogado.id:'');let chatId=[meuId,cid].sort().join('_');try{let doc=await db.collection('chats').doc(chatId).get();let msgs=doc.exists?(doc.data().mensagens||[]):[];if(msgs.length===0){c.innerHTML='<p style="color:#6B7280;">Nenhuma mensagem</p>';return;}c.innerHTML=msgs.map(m=>{let isSent=m.autorId===meuId;let bubbleClass=isSent?'sent':'received';let time=m.data?new Date(m.data).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):'';let conteudo='';if(m.audio){conteudo+='<audio controls src="'+m.audio+'" style="height:30px;width:180px;"></audio>';}if(m.imagem){conteudo+='<img src="'+m.imagem+'" class="msg-image" style="max-width:200px;border-radius:12px;cursor:pointer;" onclick="window.open(this.src)">';}if(m.texto){conteudo+=m.texto;}return '<div class="chat-bubble '+bubbleClass+'">'+conteudo+'<div class="msg-time">'+time+'</div></div>';}).join('');c.scrollTop=c.scrollHeight;}catch(e){}}
...
async function iniciarGravacaoAudio(){
    ...
    mostrarToast('🎤 Gravando áudio...','info');setTimeout(()=>{if(gravandoAudio)pararGravacaoAudio();},30000);}catch(e){mostrarToast('❌ Permita o microfone!','error');}
}
...
async function iniciarGravacaoAudioPrivado(){
    ...
    mostrarToast('🎤 Gravando...','info');setTimeout(()=>{if(gravandoAudio)pararGravacaoAudioPrivado();},30000);}catch(e){mostrarToast('❌ Permita o microfone!','error');}
}
...
// WHATSAPP
// ==========================================================
async function notificarClientesWhatsApp(){if(!barbeiroLogado)return;try{let sn=await db.collection('clientes').get();let urlLive=window.location.href.split('?')[0]+'?live=1';let notificados=0;sn.forEach(async(doc)=>{let cliente=doc.data();if(cliente.celular&&cliente.celular.length>=10){let numero=cliente.celular.replace(/\D/g,'');if(numero.length===11)numero='55'+numero;if(numero.length===10)numero='55'+numero;let mensagem=encodeURIComponent('🔴 *BARBEARIA RM - AO VIVO!*\n\nOlá '+cliente.nome+'! A Barbearia RM está ao vivo!\n\n🎥 Assista: '+urlLive+'\n\n💬 Participe do chat!\n🎉 Concorra a sorteios!');let link='https://wa.me/'+numero+'?text='+mensagem;await db.collection('notificacoes').add({clienteId:cliente.id,clienteNome:cliente.nome,tipo:'whatsapp',titulo:'📱 Convite WhatsApp',mensagem:'Link gerado para '+cliente.nome,link:link,lida:!1,data:new Date().toISOString()});notificados++;}});setTimeout(()=>{mostrarToast('📱 '+notificados+' convites gerados!','success');document.getElementById('whatsappStatus').textContent='✅ '+notificados+' links gerados!';document.getElementById('whatsappStatus').style.color='#00cc66';},2000);}catch(e){}}
async function enviarConvitesWhatsApp(){if(!barbeiroLogado)return;if(!confirm('Abrir WhatsApp para TODOS os clientes?'))return;mostrarToast('📱 Abrindo convites...','info');try{let sn=await db.collection('clientes').get();let urlLive=window.location.href.split('?')[0]+'?live=1';let contador=0;for(let doc of sn.docs){let cliente=doc.data();if(cliente.celular&&cliente.celular.length>=10){let numero=cliente.celular.replace(/\D/g,'');if(numero.length>=10){if(numero.length===11)numero='55'+numero;if(numero.length===10)numero='55'+numero;let mensagem=encodeURIComponent('🔴 *BARBEARIA RM - AO VIVO!*\n\nOlá '+cliente.nome+'! A Barbearia RM está ao vivo!\n\n🎥 Assista: '+urlLive+'\n\n💬 Participe do chat!\n🎉 Concorra a sorteios!');let link='https://wa.me/'+numero+'?text='+mensagem;if(contador<5){window.open(link,'_blank');}contador++;await new Promise(r=>setTimeout(r,300));}}}mostrarToast('📱 '+contador+' convites abertos!','success');}catch(e){}}
...
function setupEventListeners(){
    ...
    document.getElementById('btnMicPrivado')?.addEventListener('click',()=>{console.log('🎤 Mic privado clicado');if(gravandoAudio)pararGravacaoAudioPrivado();else iniciarGravacaoAudioPrivado();});
    ...
    document.getElementById('btnMic')?.addEventListener('click',()=>{console.log('🎤 Mic live clicado');if(gravandoAudio)pararGravacaoAudio();else iniciarGravacaoAudio();});
