...
async function loginBarbeiro() {
    ...
    try {
        ...
        if (barbeiro) {
            ...
            mostrarToast('❌ Dados do barbeiro não encontrados!', 'erro');
        }
    } catch (error) {
        console.error('Erro no login:', error);
        mostrarToast('❌ E-mail ou senha inválidos!', 'erro');
...
function salvarConfigPix() {
    ...
    mostrarToast('✅ Chave PIX atualizada!', 'sucesso');
    mostrarTela('homeBarbeiroScreen');
}
...
function processarPagamento() {
    ...
    if (!confirmado) {
        mostrarToast('⚠️ Confirme o pagamento antes de prosseguir!', 'erro');
        return;
    }
    ...
    mostrarToast('✅ Pagamento confirmado via ' + metodo + '!', 'sucesso');
