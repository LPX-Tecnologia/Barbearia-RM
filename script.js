...
var liveTelaAtiva = 1; // 1 = tela principal, 2 = segunda tela
var liveAnuncioAtivo = false;
var liveAnuncioImagem = '';
var liveAnuncioTempo = 0;
var liveAnuncioInterval = null;
...
// ==========================================================
// ===== LOGIN/CADASTRO (COMPLETO - MANTIDO IGUAL) =====
// ==========================================================
...
// ==========================================================
// ===== PLANOS / ANÚNCIOS / FEED / POSTS (COMPLETO) =====
// ==========================================================
...
// ==========================================================
// ===== LIVE PROFISSIONAL COM 2 TELAS E ANÚNCIOS =====
// ==========================================================
...
async function iniciarLive(){
    ...
    try{
        ...
        try{
            ...
            liveAudioMonitor=new(window.AudioContext||window.webkitAudioContext)({sampleRate:48000});
            var source=liveAudioMonitor.createMediaStreamSource(stream);
            var gainNode=liveAudioMonitor.createGain();gainNode.gain.value=0.5;
        ...
        document.getElementById('liveVideoLocal').muted=true;
        await db.collection('lives').doc('live_atual').set({id:'live_atual',barbeiroId:barbeiroLogado.id,barbeiroNome:barbeiroLogado.nome,titulo,ativa:true,chat:[],viewers:0,totalViews:0,likes:0,anuncioAtivo:false,anuncioImagem:'',dataInicio:new Date().toISOString()});
        liveAtiva=true;liveChatMessages=[];liveLikes=0;liveLiked=false;liveViewersAtivos={};liveUltimoFrameEnviado=0;liveTelaAtiva=1;liveAnuncioAtivo=false;
        ...
        liveFrameCanvas=document.createElement('canvas');liveFrameCanvas.width=640;liveFrameCanvas.height=360;
...
function iniciarCapturaFrames(){
    ...
    liveFrameInterval=setInterval(async function(){
        ...
        try{
            ...
            var agora=Date.now();if(agora-liveUltimoFrameEnviado<250)return;liveUltimoFrameEnviado=agora;
            liveFrameCtx.drawImage(video,0,0,640,360);
            var frameData=liveFrameCanvas.toDataURL('image/jpeg',0.85);
...
function alternarTela(){
    if(!liveAtiva||!barbeiroLogado)return;
    liveTelaAtiva=liveTelaAtiva===1?2:1;
    var indicador=document.getElementById('liveTelaIndicador');
    if(indicador)indicador.textContent='📺 TELA '+liveTelaAtiva;
    mostrarToast('📺 Tela '+liveTelaAtiva+' ativada','info');
}

// --- ANÚNCIO NA LIVE ---
function ativarAnuncioNaLive(){
    if(!liveAtiva||!barbeiroLogado)return;
    document.getElementById('anuncioLiveInput').click();
}
...
function carregarAnuncioLive(event){
    ...
    reader.onload=function(e){
        liveAnuncioImagem=e.target.result;
        document.getElementById('liveAnuncioImg').src=liveAnuncioImagem;
        document.getElementById('liveAnuncioOverlay').style.display='flex';
        liveAnuncioAtivo=true;
        db.collection('lives').doc('live_atual').update({anuncioAtivo:true,anuncioImagem:liveAnuncioImagem});
        mostrarToast('📢 Anúncio exibido na live!','success');
        // Auto-remover após 10 segundos
        if(liveAnuncioInterval)clearTimeout(liveAnuncioInterval);
        liveAnuncioInterval=setTimeout(function(){removerAnuncioLive();},10000);
    };
...
function removerAnuncioLive(){
    document.getElementById('liveAnuncioOverlay').style.display='none';
    liveAnuncioAtivo=false;liveAnuncioImagem='';
    db.collection('lives').doc('live_atual').update({anuncioAtivo:false,anuncioImagem:''});
    if(liveAnuncioInterval)clearTimeout(liveAnuncioInterval);
}
...
async function carregarLive(){
    ...
    try{
        ...
        if(doc.exists&&doc.data().ativa){
            ...
            // Verificar anúncio
            if(live.anuncioAtivo&&live.anuncioImagem&&clienteLogado){
                document.getElementById('liveAnuncioImg').src=live.anuncioImagem;
                document.getElementById('liveAnuncioOverlay').style.display='flex';
            }
